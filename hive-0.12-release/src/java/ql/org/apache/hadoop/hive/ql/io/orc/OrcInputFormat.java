...
public class OrcInputFormat  extends FileInputFormat<NullWritable, OrcStruct>
  ...
  try{
      ...
      //读取的列的名称，select 后面跟着的列 ，用 ,号分割 ，如果存在pushdown的话，是否这个列中会将pushdown相关的Predicate也一并包含进来  
      ...
      //orcfile中读出来的所有列存储在types[1--->size-1]中间，使用Type的subtypes_(List<Intgeer>)支撑起来一个树形的结构
      //目前因为都是基本类型，所以整棵树就两层types[0](STRUCT ,sub是所有列) ，types[1--->size-1] ：所有列  
      ...
      if (serializedPushdown != null && columnNamesString != null) {
        ...
        int i = 0;
        //columnNames中相应位置，放上column name ，columnNames 和 types对应  
   ...
   * 都包含的时候返回null , 只是需要部分包含的时候返回一个boolean[column_num]的数组
   */
