...
//一个具体的谓词表达式,类似于：column1<100  ,column1 是 int类型 
  static final class PredicateLeafImpl implements PredicateLeaf {
    private final Operator operator;  //比较符号， < 
    private final Type type;          //colnum类型，后面和index信息比较的时候，会根据这个做相应的类型转换，int   
    private final String columnName;  //列名 ，column1
    private final Object literal;     //具体的参数 ,100 
    private final List<Object> literalList; //对于多个参数时候用到，比如between 、in 操作 
  ...
  static class ExpressionTree {
    ...
    //操作类型，或、与、非
    //LEAF：表示是叶子节点，是一个PredicateLeafImpl具体的谓词判断，此时leaf！=-1 ，SearchArgumentImpl.leaves[leaf]即是相应的谓词判断节点
    //---PredicateLeafImpl可以根据orcindex的min/max算出自己的结果值 TruthValue，看是否需要过滤掉这个块 
    //CONSTANT :一些在表中无法下推的谓词条件（例如column1 >column2之类）都是这节设置成CONSTANT，
    //--- 其TruthValue constant=YES_NO_NULL ，即不对结果产生影响  
    ...
    /**
     * 在整个SearchArgumentImpl中将所有的叶子节点做成一个数组，数组的计算结果就是 TruthValue[] leaves
     * SearchArgumentImpl的逻辑判断树(就是那些or and 等组合起来的条件)放在ExpressionTree expression中，
     * 一颗逻辑判断树，子节点也是ExpressionTree ，
     * 若是叶子节点的话则private final int leaf ，leaf!=-1 ,SearchArgumentImpl.leaves.get(leaf) 就是该叶子节点的PredicateLeaf
     * @param leaves   
     * @return
     */
  ...
  static class ExpressionBuilder {
    ...
    private int findVariable(ExprNodeDesc expr) {
      ...
      for(int i = 0; i < children.size(); ++i) {
        ...
        if (child instanceof ExprNodeColumnDesc) {
          ...
          //一个表达式中含有多个列的时候，过滤掉这个，例如 ：colum1 > column2 这样的逻辑运算  
          if (result != -1) {
            return -1;
          } else {
            result = i;
     ...
     * 将语法树种所有的的节点ExprNodeDesc转换为 PredicateLeaf  
