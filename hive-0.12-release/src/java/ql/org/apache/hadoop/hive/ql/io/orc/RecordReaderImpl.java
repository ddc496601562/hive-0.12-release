...
import org.apache.hadoop.hive.ql.io.sarg.TestStatic;
...
class RecordReaderImpl implements RecordReader {
  ...
  //每个index的数据行数，由orc.row.index.stride参数设置
  ...
  RecordReaderImpl(Iterable<StripeInformation> stripes,
    ...
    totalRowCount = rows;
    //这个included只是上层传进来的include，其它的列则没有reader  
  ...
  /**
   * 每个列都是通过BitFieldReader present来优化null值得存储
   * @author dingdongchao
   *
   */
  ...
  //root为StructTreeReader ,其TreeReader[] fields为各个需要的列的Reader ，若是基本类型，则都是相应的reader 
  ...
  //和统计信息的min、max比较的位置  ---before-------min------middle------max-----after---
   ...
   * 如果都需要包含的话，返回null 
   * 有skip的时候才返回一个boolean[index_num]的数组  
   * @throws IOException
   */
  private boolean[] pickRowGroups() throws IOException {
    ...
    //根据sarg计算每个index索引的小块是否应该在结果中被包含
    for(int rowGroup=0; rowGroup < result.length; ++rowGroup) {
    	//每个rowGroup 
      //计算每个Predicate在当前rowGroup下的逻辑值 
      ...
      //判断该rowGroup是否应该被过滤掉 ？sarg中的  Predicate条件 在这个rowGroup上是否成立 ？
      //true表示被选中保留
    ...
    //no rowGroup should keep   
    for(boolean b: result) {
      if (!b) {
        return result;
      }
    }
    //for test,ranomd chose index-block data ,probability is 1/*****  
...
//    }
  ...
  private void readAllDataStreams(StripeInformation stripe
    ...
    for(OrcProto.Stream section: stripeFooter.getStreamsList()) {
      //目前对于基本类型，在data数据中只有  DATA LENGTH DICTIONARY_DATA LENGTH 四种 
  ...
  static List<DiskRange> planReadPartialDataStreams
    ...
    // this  means that   some columns has not a  present stream when writting ,如果全是not null的话，则没有present 
  ...
  private void readPartialDataStreams(StripeInformation stripe
    ...
    TestStatic.readTimes.add(chunks.size());
    //将分段的数据读取到byte[][] bytes一个二维byte[]数组中   
  ...
  private void advanceToNextRow(long nextRow) throws IOException {
	  //调用的使用+rowBaseInStripe ，这里 - rowBaseInStripe  ,why ???
