...
class WriterImpl implements Writer, MemoryManager.Callback {
  ...
  private static class StringTreeWriter extends TreeWriter {
    ...
    //整个String的存储有可能使用字典的形式，有可能使用直接写的形式   ，由字典的个数/非控制数 的比来来控制，若是这个比例太大，其实没必要用字典了  
    ...
    StringTreeWriter(int columnId,
      ...
      lengthOutput = createIntegerWriter(writer.createStream(id,OrcProto.Stream.Kind.LENGTH), false, isDirectV2);
    ...
    void writeStripe(OrcProto.StripeFooter.Builder builder,
      ...
      useDictionaryEncoding =
        (!isDirectV2) || 
        (rows.size() > 0 &&(float)(dictionary.size()) / rows.size() <= dictionaryKeySizeThreshold);
