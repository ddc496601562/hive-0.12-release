...
public interface SearchArgument {
   ...
   * 对于一般的数据库中谓词的比较操作 
   * NULL < /> />= /= /<=等操作均为 NULL   
   * NULL IS NULL 为YES ,OTHER IS NULL 为 NO  
   * 定义了谓词比较操作的7中结果，一般是谓词比较形式是：column PredicateLeaf.Operator  literal 
   * 通过比较literal 和 统计信息的min 、max得出以下7种结果 
   * 全都符合条件的
   * 全部不符合的
   * 比较的结果全是NULL
   * 除了比较结果是NULL的全是符合条件的
   * 除了比较结果是NULL的全是不符合条件的
   * 一部分是符合条件的，一部分是不符合条件的
   * 除了结果是NULL的，有些符合有些不符合  
   * 
   */
