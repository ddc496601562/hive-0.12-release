...
public class OrcOutputFormat extends FileOutputFormat<NullWritable, OrcSerdeRow>
  ...
  public static class OrcRecordWriter
    ...
    public OrcRecordWriter(Path path, OrcFile.WriterOptions options) {
      this.path = path;
    ...
    public void write(NullWritable nullWritable,
        ...
        //延迟创建基本写的Write ，写的时候需要的WritableValue是 OrcSerdeRow 
      if (writer == null) {
  ...
  public FileSinkOperator.RecordWriter
     getHiveRecordWriter(JobConf conf,
                         ...
                         Progressable reporter) throws IOException {
	 //*****************************************************************************************
	  System.out.println("use getHiveRecordWriter method !!");
	  try{	
	  PrintStream out=new PrintStream("/home/work/local/hive-0.12.0-bin/bin/output.conf");
	  	Iterator<Entry<String, String>> iterator=conf.iterator();
	  	while(iterator.hasNext()){
	  	...
	  	out=new PrintStream("/home/work/local/hive-0.12.0-bin/bin/tableProperties.conf");
...
//*****************************************************************************************
  ...
  //设置orcfile的一些参数 ，详见配置项 http://docs.hortonworks.com/HDPDocuments/HDP2/HDP-2.0.0.2/ds_Hive/orcfile.html#ORCFiles-HiveQLSyntax
