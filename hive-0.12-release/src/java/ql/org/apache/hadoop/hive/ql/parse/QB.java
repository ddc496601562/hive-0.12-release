...
public class QB {
  ...
  //别名--->table名称的映射，没有别名则是本身到table名的映射  ,这个只有在from中处理具体的tableref的时候才会添加
  private HashMap<String, String> aliasToTabs;
  //对于别名到--->子查询的映射，子查询其实也是一个相同的from ****  insert  **** 的sql ，同一层中tok_from后面有且只有一个子查询，而且和上面的那个直接和table的映射是对立的，要么是子查询要么是table  
